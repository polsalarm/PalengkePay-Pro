...
| API functions | Fee bump sponsorship, stable checkout quotes, and health checks | `frontend/api/fee-bump.ts`, `frontend/api/quote.ts`, `frontend/api/health.ts` |
...
| --- | --- | --- | --- | --- |
| Live QR payments | `PalengkePayment` contract when configured | Contract record/event plus Horizon/local cache fallback | `prepareContractTx()` -> wallet sign -> `submitSorobanTx()` | Direct payment remains missing-contract fallback |
...
| Admin metrics | `PalengkePayment` records preferred | `useMetrics` -> `payment-source.ts` -> `get_vendor_payments` | QR payments through `PalengkePayment.pay` | Falls back to registry counters if payment reads fail |
| Customer/vendor payment history | `PalengkePayment` records preferred | `useTransactions` -> `payment-source.ts`; Horizon/localStorage fallback | QR payments through `PalengkePayment.pay` | Customer contract lookup requires redeploy |
...
Customer scans vendor QR
  -> PaymentForm collects PHP amount/memo
  -> quote API/helper locks PHP/XLM rate and XLM settlement amount
  ...
  -> payment-source.ts reads contract records for history and metrics
  -> Horizon/indexer remains fallback cache
```
...
| `PalengkePayment` | Payment records/events in contract code | `pay` | `get_payment`, `get_vendor_payments`, `get_customer_payments`, `payment_count` |
...
Metrics, history, future credit scoring, and admin dashboards now have a shared frontend payment-source layer. Drift can still happen if production keeps using an older deployed `PalengkePayment` contract without `get_customer_payments`, or if registry fallback remains enabled for too long.
...
Customer scans QR
  ...
  -> Admin metrics, history, and PalengkeScore read from that canonical source
...
- Payment events, history, and metrics come from the same transaction path.
- Horizon/localStorage is cache only.
- `VendorRegistry.increment_stats` is retired or restricted to an authorized payment-contract/event-normalizer path.
...
- `PalengkePayment` records are the intended authority for QR payment history and metrics.
...
- Admin metrics use payment records first and legacy registry counters only as fallback.
...
1. Decide whether vendor stats live only in `PalengkePayment` reads or are copied into `VendorRegistry` through an authorized contract/event bridge.
