...
- **Credit RWA — Score-Gated Working Capital + Scheduled Auto-Repay** — on-chain cashflow becomes a 300–850 credit score that unlocks a USDC/XLM lending pool (`CreditPool`); vendors repay manually or opt into scheduled auto-repay — a permissionless relayer pulls a capped, vendor-approved SEP-41 allowance on a Daily/Weekly/Monthly cadence, never custodying funds. Live on Testnet and Mainnet — scoring formula + full design in [`CREDIT_RWA.md`](CREDIT_RWA.md), mainnet contract IDs below
...
Credit RWA layer (`CreditPool` USDC + XLM pools, registry v2) deploys separately — contract IDs and the auto-repay flow are documented in [`CREDIT_RWA.md`](CREDIT_RWA.md).
